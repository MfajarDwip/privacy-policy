...
public function getPendudukByUserId(Request $request)
{
    ...
    if ($user) {
        ...
        // Temukan data penduduk berdasarkan ID pengguna
        $penduduk = Penduduk::where('id_user', $userId)->first();

        if ($penduduk) {
            // Jika data penduduk ditemukan, kembalikan respons
            return response()->json([
                'status' => true,
            ...
            // Jika data penduduk tidak ditemukan, kembalikan respons dengan pesan error
            return response()->json([
                'status' => false,
                'message' => 'Penduduk not found for this user',
                'penduduk' => null
            ], 404);
        }
    } else {
        // Jika pengguna tidak ditemukan, kembalikan respons dengan pesan error
        return response()->json([
            'status' => false,
            'message' => 'User not found',
            'penduduk' => null
    ...
    public function getUserFromToken(Request $request)
    {
        try {
            ...
            if ($user) {
                ...
                // Ambil data penduduk jika tersedia
                $penduduk = Penduduk::where('id_user', $user->id)->first();
                if ($penduduk) {
                    $userData['penduduk'] = $penduduk;
                }
